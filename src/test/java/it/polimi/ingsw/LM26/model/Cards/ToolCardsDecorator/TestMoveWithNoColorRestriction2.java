...
public class TestMoveWithNoColorRestriction2 {
    ...
    public void checkEffect() {
        ...
        if(!board[i][j].isIsPresent()) assertFalse(model.getDecks().getToolCardDeck().get(1).play(board[i][j], board[0][0], player.getIDPlayer()));
        ...
        while(!model.getDecks().getToolCardDeck().get(1).play(board[0][0], board[i][j], player.getIDPlayer())) {
            i = rand.nextInt(4);
            j = rand.nextInt(5);
            System.out.println("trying with "+i+" "+j);
        }
        player.getPlayerBoard().printCard();
        if(board[i][j].getPatternBox().isShade())assertEquals(die1.getValue(), board[i][j].getPatternBox().getValue());
        ...
        i = rand.nextInt(4);
        j = rand.nextInt(5);
        while(! model.getDecks().getToolCardDeck().get(1).play(board[1][1], board[i][j], player.getIDPlayer())){
            i = rand.nextInt(4);
            j = rand.nextInt(5);
            System.out.println("trying with "+i+" "+j);
        }
